...
function initICPQuery() {
  var icpToggle = document.getElementById('icp-toggle');
  var icpParamsPanel = document.getElementById('icp-params-panel');
  if (!icpParamsPanel) return;

  if (icpToggle) {
    icpToggle.addEventListener('change', function() {
      icpParamsPanel.style.display = icpToggle.checked ? 'block' : 'none';
    });
  }

  var icpSearchBtn = document.getElementById('btn-icp-search');
  if (!icpSearchBtn) return;

  icpSearchBtn.addEventListener('click', function() {
    var icpType = document.getElementById('icp-type').value;
    var searchInput = document.getElementById('icp-search-input');
    if (!searchInput || !searchInput.value.trim()) {
      showMessage('请输入域名、公司名或许可证号', 'warning');
      return;
    }
    var search = searchInput.value.trim();

    icpSearchBtn.textContent = '查询中...';
    icpSearchBtn.disabled = true;

    fetch('/api/icp/query?type=' + encodeURIComponent(icpType) + '&search=' + encodeURIComponent(search))
      .then(function(resp) { return resp.json(); })
      .then(function(data) {
        icpSearchBtn.textContent = 'ICP查询';
        icpSearchBtn.disabled = false;
        if (data.error) {
          showMessage('ICP查询失败: ' + data.error, 'error');
          return;
        }
        renderICPResults(data);
      })
      .catch(function(err) {
        icpSearchBtn.textContent = 'ICP查询';
        icpSearchBtn.disabled = false;
        showMessage('ICP查询请求失败: ' + err.message, 'error');
      });
  });
}

function renderICPResults(data) {
  var resultsSection = document.getElementById('icp-search-results');
  var tbody = document.getElementById('icp-results-body');
  ...
  if (results.length === 0) {
    showMessage('未找到ICP备案信息', 'warning');
    resultsSection.style.display = 'none';
    return;
  }

  results.forEach(function(r) {
    // Support both web (domain-based) and app/mapp/kapp (serviceName-based) results
    var domain = r.domain || r.icp_domain || '-';
    var title = r.contentTypeName || r.contentName || r.serviceName || r.icp_service_name || '-';
    var unit = r.unitName || r.org || '-';
    var licence = r.licence || r.mainLicence || r.serviceLicence || r.icp_licence || r.icp_main_licence || '-';
    var nature = r.natureName || r.icp_nature || '-';
    var city = r.cityName || r.icp_city || '-';
    var svcType = r.serviceType || r.icp_service_type || '-';
    var updateTime = r.updateRecordTime || r.updateRecord || r.icp_update_record || '-';
    ...
    tr.innerHTML = '<td>' + escapeHTML(domain) + '</td>'
      ...
      + '<td>' + escapeHTML(svcType) + '</td>'
      + '<td>' + escapeHTML(updateTime) + '</td>';
    tbody.appendChild(tr);
  });

  resultsSection.style.display = 'block';
  showMessage('查询到 ' + results.length + ' 条ICP备案信息', 'success');
}
