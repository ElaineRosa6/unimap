# 0005: Bridge API 独立鉴权边界

**日期:** 2026-05-09
**状态:** 已采纳，2026-07-13 按当前实现补充
**来源:** `slice2-bridge-auth-contract.md`

## 背景

Chrome 扩展桥接 API（`/api/v1/screenshot/bridge/*`）需要独立的身份验证，不能与 Web 管理 session 混用。
...
5. **Token 生命周期** — TTL + 轮换 + 撤销；配对、轮换、任务拉取与回调仅允许 loopback
6. **审计日志** — 所有 token 操作记录审计日志
7. **脱敏状态** — Bridge 状态接口返回脱敏信息，不返回完整 token
8. **截图模式** — 当前 API 支持 `cdp` / `extension` / `auto`，由 ScreenshotRouter 依据健康状态降级
...
- 不存在 `/diagnostic` Bridge 端点；本机诊断使用 `health` 与 `status`
